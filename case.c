...
void casedouane( int de[3], int douane, t_joueur tabdejoueur[6], int i)
{
    int choix;
    int verif=1;
    do
...
{
    int a;
    if (plat[tabdejoueur[i].position[1]].type==VILLE)
    {
        gotoligcol(51+ligne,10);
        printf("vouys etes sur une ville");
        scanf("%c",&a);
        //acheter case ou payer
    }
    else if (plat[tabdejoueur[i].position[1]].type==GARE)
    {
        gotoligcol(52+ligne,10);
        printf("vous etes sur une gare");
        // acheter case ou payer
    }
    else if (plat[tabdejoueur[i].position[1]].type==CHANCE)
    {
        casechance(tabdejoueur,i,plat,ligne, carte);
    }
    else if (plat[tabdejoueur[i].position[1]].type==COMM)
    {
        casedecommunaute(ligne,i,plat,tabdejoueur, carte);
    }
    else if (plat[tabdejoueur[i].position[1]].type==PRISON)
    {
        tabdejoueur[i].position[1]=8;
        gotoligcol(10+ligne+i,146);
        printf("%c",tabdejoueur[i].pionjoueur);
    }
    else if (plat[tabdejoueur[i].position[1]].type==IMPOT)
    {
        carteimpot(i,tabdejoueur);
    }
}
